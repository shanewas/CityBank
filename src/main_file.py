...
from .nid_stuff import login_nid, get_nid_detail
from .cib_stuff import cib_login, cib_fill_form, gettinglogs
...
def push_data(nid, task, result):
    # with open('logs.csv', mode='w') as csv_file:
    #     fields_na
    #     me = ['time', 'task', 'result']
    #     writer = csv.DictWriter(csv_file, fields_name)
    #     data_dict = {}
    #     for value in range(len(fields_name)):
    #         data_dict[fields_name[value]] = fields_name[value]
    # changes = {}
    # changes['time'] = '5:50'
    # changes['task'] = task
